...
import scripts.FmAPI.FmConditions;
 ...
 * additional condition work done by Fmtrick
 */
public class FmHopper
{
	//constants
			...
			try
			{
				...
				if(isWorldVisible(TARGET_WORLD) || scrollWorldIntoView(WORLD_LIST, TARGET_WORLD))
					return clickWorld(WORLD_LIST.getChild(WORLD_CHILDREN_CACHE.get(world)), world) //Can't use cached TARGET_WORLD due to it's pos changing on scroll
							&& Timing.waitCondition(FmConditions.IN_GAME_CONDITION, 6000);
			}
			catch(Exception e){}
		}
	...
	private static boolean openHopper()
	{
		...
		if(WORLD_SWITCHER_BUTTON != null && Clicking.click(WORLD_SWITCHER_BUTTON))
			return Timing.waitCondition(FmConditions.interfaceUp(HOPPER_MASTER), INTERFACE_SWITCH_TIMEOUT) &&
					Timing.waitCondition(FmConditions.interTextNotContains(HOPPER_MASTER, LOADING_TEXT_CHILD, "Loading..."), INTERFACE_LOAD_TIMEOUT);
	...
	public static boolean closeHopper()
	{
		...
		if(CLOSE_BUTTON != null && Clicking.click(CLOSE_BUTTON))
			return Timing.waitCondition(FmConditions.interfaceNotUp(HOPPER_MASTER), 1500);
	...
	{
		if(Clicking.hover(targetWorld) && Timing.waitCondition(FmConditions.uptextContains(""+worldNum), 1000))
		{
			Mouse.click(1);
			if(Timing.waitCondition(FmConditions.interfaceUp(DIALOGUE_MASTER), 1500))
			{
			...
			return Timing.waitCondition(FmConditions.onWorldCondition(worldNum), 5000);
		}
